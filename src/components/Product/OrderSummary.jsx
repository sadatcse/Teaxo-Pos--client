...
const OrderSummary = ({
    ...
    const handleFinalizeOrder = (actionCallback, withPrint) => {
        ...
        // Directly proceed to the action (e.g., printing or saving).
                            ...
                            <div className="mt-6 p-4 rounded-xl">
                                ...
                                <div className="flex justify-center gap-3">
                                    {["Cash", "Card", "Mobile", "Bank"].map((method) => (
                                        <button key={method} onClick={() => handleMainPaymentButtonClick(method)} className={`min-w-[100px] px-4 py-3 rounded-xl font-bold flex items-center justify-center gap-2 text-sm ${selectedPaymentMethod === method || (selectedPaymentMethod && cardOptions.map(o => o.name).includes(selectedPaymentMethod) && method === 'Card') || (selectedPaymentMethod && mobileOptions.map(o => o.name).includes(selectedPaymentMethod) && method === 'Mobile') || (selectedPaymentMethod === 'Bank' && method === 'Bank') ? "bg-blue-600 text-white shadow-lg" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`} disabled={isProcessing}>
                                ...
                                {selectedPaymentMethod === 'Card' && (<div className="mt-4 flex flex-wrap justify-center gap-3">{cardOptions.map((card) => (<button key={card.name} onClick={() => handleSubPaymentButtonClick(card.name, card.icon)} className={`px-4 py-2 rounded-xl font-bold flex items-center justify-center gap-2 text-sm ${selectedSubMethod === card.name ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`} disabled={isProcessing}>{card.icon}<span>{card.name}</span></button>))}</div>)}
                                {selectedPaymentMethod === 'Mobile' && (<div className="mt-4 flex flex-wrap justify-center gap-3">{mobileOptions.map((mobile) => (<button key={mobile.name} onClick={() => handleSubPaymentButtonClick(mobile.name)} className={`px-4 py-2 rounded-xl font-bold flex items-center justify-center gap-2 text-sm ${selectedSubMethod === mobile.name ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`} disabled={isProcessing}><span>{mobile.name}</span></button>))}</div>)}
                            </div>
                    ...
                    <div>
                        ...
                        {user?.role === 'admin' && (
                            <div className="mt-6 p-4 border-2 border-dashed border-orange-400 rounded-xl bg-orange-50 text-orange-800 shadow-sm">
                                <h3 className="font-bold text-md mb-2 text-center text-orange-900">Admin: Custom Order Entry</h3>
                                <div className="flex flex-col">
                                    <label htmlFor="customDateTime" className="block text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                                        <FaClock className="text-orange-600" />Custom Order Date & Time:
                                    </label>
                                    <input id="customDateTime" type="datetime-local" className="border border-gray-300 px-3 py-2 rounded-xl focus:ring-orange-500 focus:border-orange-500 transition-all duration-300" value={customDateTime} onChange={(e) => setCustomDateTime(e.target.value)} />
                                    <p className="text-xs text-gray-500 mt-1">For back-dated entries only. Leave empty to use the current time.</p>
                                </div>
                            </div>
                        )}
                    </div>
