...
const Purchases = () => {
    ...
    }, [branch, fetchPurchases, fetchPrerequisites]);
    ...
    const handleSubmit = async () => {
        if (!user || !user._id) {
            Swal.fire({ icon: 'error', title: 'Authentication Error!', text: 'User data is not available. Please try logging in again.' });
            return;
        }
        setFormLoading(true);
        ...
        try {
            const apiCall = editId
                ? axiosSecure.put(`/purchase/update/${editId}`, payload)
                : axiosSecure.post('/purchase/post', payload);
            await apiCall;
            fetchPurchases();
            closeModal();
    ...
    const handleDelete = (purchaseId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                if (!user || !user._id) {
                    Swal.fire('Error!', 'Could not authenticate user for deletion.', 'error');
                    return;
                }
                try {
        ...
        <div className="p-4 sm:p-6 lg:p-8 min-h-screen bg-base-200">
            <Mtitle title="Purchase Management" rightcontent={
                <div className='flex flex-col md:flex-row md:items-center gap-4'>
                    <input type="date" value={dateFilter} onChange={(e) => setDateFilter(e.target.value)} className="border border-gray-300 rounded-xl p-2 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150" />
                    <div className='relative w-full md:w-64'>
                        <TfiSearch className='absolute top-1/2 left-3 -translate-y-1/2 text-gray-400' />
                        <input type="text" className='border border-gray-300 rounded-xl p-2 pl-10 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150 w-full' placeholder='Search...' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    </div>
                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={openCreateModal} className="flex items-center gap-2 bg-blue-600 text-white py-2 px-4 rounded-xl shadow-md hover:bg-blue-700 transition duration-300 w-full md:w-auto justify-center">
                                ...
                                <tbody>
                                    <AnimatePresence>
                                        {paginatedData.length === 0 ? (
                                            <tr><td colSpan="8" className="text-center py-10 text-slate-700">No purchases found.</td></tr>
                                        ) : (
                                            paginatedData.map((p) => (
            ...
            <AnimatePresence>
                {isModalOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-base-100 rounded-xl shadow-xl w-full max-w-5xl max-h-full overflow-y-auto">
                            <div className="p-6">
                                <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-200">
                                    <h2 className="text-xl font-semibold text-blue-600">{editId ? 'Edit Purchase' : 'Create a New Purchase'}</h2>
                                    <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={closeModal} className="btn btn-ghost btn-circle"><FiX className="text-xl" /></motion.button>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                    <select name="vendor" value={formData.vendor} onChange={handleFormChange} className="border border-gray-300 rounded-xl p-2 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150 w-full" required>
                                        <option value="" disabled>Select Vendor</option>
                                        {vendors.map(v => <option key={v._id} value={v._id}>{v.vendorName}</option>)}
                                    </select>
                                    <input type="text" name="invoiceNumber" value={formData.invoiceNumber} onChange={handleFormChange} className="border border-gray-300 rounded-xl p-2 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150 w-full" placeholder="Invoice No *" required />
                                    <DatePicker selected={new Date(formData.purchaseDate)} onChange={handleDateChange} dateFormat="dd/MM/yyyy" className="border border-gray-300 rounded-xl p-2 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150 w-full" required disabled={user?.role !== 'admin'} />
                                </div>
                                <h3 className="text-xl font-semibold text-blue-600 mb-3 mt-8">Purchase Items</h3>
                                <div className="overflow-x-auto rounded-lg border">
                                    <table className="w-full">
                                        <thead className="bg-slate-100">
                                            <tr>{['Category', 'Ingredient', 'Quantity', 'Unit Price', 'Total', 'Action'].map(h => <th key={h} className="p-3 text-left text-sm font-semibold text-slate-700 tracking-wider">{h}</th>)}</tr>
                                        </thead>
                                        <tbody>
                                            {formData.items.map((item, index) => (
                                                <tr key={index} className="border-b last:border-0">
                                                    <td className="p-2"><select value={itemCategories[index]} onChange={e => handleCategoryChange(index, e.target.value)} className="border border-gray-300 rounded-xl p-2 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150 w-full" required><option value="" disabled>Select</option>{categories.map(c => <option key={c._id} value={c._id}>{c.categoryName}</option>)}</select></td>
                                                    <td className="p-2"><select name="ingredient" value={item.ingredient} onChange={e => handleItemChange(index, e)} className="border border-gray-300 rounded-xl p-2 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150 w-full" required disabled={!itemCategories[index]}><option value="" disabled>Select</option>{ingredients.filter(i => i.category?._id === itemCategories[index]).map(i => <option key={i._id} value={i._id}>{i.name} ({i.unit})</option>)}</select></td>
                                                    <td className="p-2"><input type="number" name="quantity" value={item.quantity} onChange={e => handleItemChange(index, e)} className="border border-gray-300 rounded-xl p-2 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150 w-full" required /></td>
                                                    <td className="p-2"><input type="number" name="unitPrice" value={item.unitPrice} onChange={e => handleItemChange(index, e)} className="border border-gray-300 rounded-xl p-2 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150 w-full" required /></td>
                                                    <td className="p-2"><input type="text" value={(item.totalPrice || 0).toFixed(2)} className="border border-gray-300 rounded-xl p-2 shadow-sm bg-slate-100 w-full" readOnly /></td>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                                    <div>
                                        <label className="label text-sm text-slate-700">Notes</label>
                                        <textarea name="notes" value={formData.notes} onChange={handleFormChange} className="border border-gray-300 rounded-xl p-2 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150 w-full h-32" placeholder="Add any relevant notes..."></textarea>
                                    </div>
                                    <div className="bg-slate-50 p-4 rounded-lg flex flex-col justify-between">
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex justify-between items-center"><label className="label text-sm text-slate-700">Paid Amount *</label><input type="number" name="paidAmount" value={formData.paidAmount} onChange={handleFormChange} className="border border-gray-300 rounded-xl p-2 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150 w-40 text-right" required /></div>
                                            <div className="flex justify-between items-center"><label className="label text-sm text-slate-700">Payment Method *</label><select name="paymentMethod" value={formData.paymentMethod} onChange={handleFormChange} className="border border-gray-300 rounded-xl p-2 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150 w-40 text-right" required><option>Cash</option><option>Card</option><option>Mobile</option><option>Other</option></select></div>
                                        </div>
            ...
            <AnimatePresence>
                {isViewModalOpen && viewingPurchase && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-base-100 rounded-xl shadow-xl w-full max-w-3xl max-h-full overflow-y-auto">
                            <div className="p-6">
                                <div className="flex justify-between items-center mb-5 pb-4 border-b border-slate-200">
                                    <h2 className="text-xl font-semibold text-blue-600">Purchase Details</h2>
                                    <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={closeModal} className="btn btn-ghost btn-circle"><FiX className="text-xl" /></motion.button>
                                </div>
                                ...
                                <h3 className="text-xl font-semibold text-blue-600 mb-2">Items Purchased ({viewingPurchase.items.length})</h3>
                                ...
                                <div className="text-right mt-8 pt-6 border-t border-slate-200">
                                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={closeModal} className="btn rounded-xl">Close</motion.button>
                                </div>
                            </div>
                        </motion.div>
                    </motion.div>
