...
const Step6Users = ({ data, onNext, onPrev }) => {
    const [users, setUsers] = useState(data.users.length > 0 ? data.users : [{ name: '', email: '', password: '', role: 'manager' }]);

    // Combine default roles with custom roles from the previous step
    const availableRoles = useMemo(() => {
        const defaultRoles = ['manager', 'user'];
        const customRoles = data.roles.map(r => r.roleName.toLowerCase());
        // Use a Set to ensure all role names are unique
    ...
    return (
        <div>
            ...
            <p className="text-sm text-gray-500 mb-4">Create at least one Manager for the new branch. Custom roles are available in the dropdown.</p>
            ...
            <div className="flex justify-between mt-6">
                ...
                <button type="button" onClick={() => onNext({ users })} className="btn bg-blue-600 hover:bg-blue-700 text-white">Next <FiArrowRight/></button>
            </div>
        </div>
    );
};

export default Step6Users;